...
//----------------------------------------------------------------------------------------
float SHT1x_CalcTemperature(float *tempReading)
//----------------------------------------------------------------------------------------
{
    const float D1 = -39.66;        // For 3.3 volt power supply, centigrade
    const float D2 = 0.01;          // For 14 bit temperatures, centigrade

    return D1 + (D2 * *tempReading);
}

//----------------------------------------------------------------------------------------
float SHT1x_CalcHumidity(float *humidityReading, float tempInCelcius)
//----------------------------------------------------------------------------------------
{
    const float C1=-2.0468;            		// for 12 Bit
	const float C2=+0.0367;           		// for 12 Bit
    ...
    // calculate the %RH from the humidity reading
    float percentRelativeHumidity = C1 + (C2 * *humidityReading) + (C3 * *humidityReading * *humidityReading);

    // and compensate for the current temperature
    return (tempInCelcius - 25) * (T1 + (T2 * percentRelativeHumidity)) + percentRelativeHumidity;
}
